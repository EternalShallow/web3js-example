  ...
  methods: {
    ...
    async getBalanceInfo () {
      try {
        ...
        that.token_info.balance_amount_origin = that.$web3_http.utils.fromWei(balance_amount, that.wei)
        ...
        that.token_info.total_amount_origin = that.$web3_http.utils.fromWei(total_amount + '', that.wei)
        ...
        that.earn_amount = parseFloat(keepPoint(that.$web3_http.utils.fromWei(available + '', that.wei), 6))
        // 待取收益
        const unclaimed_profit = await that.$VAULT.methods.cal_out(that.$account).call()
        that.token_info.unclaimed_profit_origin = that.$web3_http.utils.fromWei(unclaimed_profit, 'ether')
        ...
        that.token_info.deposit_amount_origin = that.$web3_http.utils.fromWei(player.stake, that.wei)
    ...
    async approve () {
      console.log(await that.$web3_http.eth.getGasPrice())
    }
  }
}
